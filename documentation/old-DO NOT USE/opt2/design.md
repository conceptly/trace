...
All colors are defined as HiFi design tokens. The mid-fi prototype uses a neutral approximation palette; Webflow implementation should use the HiFi values below.

### 3.1 HiFi Brand Palette

| Role | Token Name | Hex | Notes |
|---|---|---|---|
| Outer Canvas | `bg-canvas` | `#0F2229` | Deepest dark — app shell / outer background |
| Background | `bg-base` | `#223740` | Primary dark background |
| Background Card | `bg-card` | `#2D4855` | Elevated card surfaces |
| Primary Elements | `text-emphasis` | `#EDF4F0` | H1–H3, icons, primary UI |
| Body Copy | `text-body` | `#BDD0CC` | Paragraph text |
| Secondary Elements | `text-secondary` | `#8AADB5` | Captions, metadata |
| Primary CTA | `cta-default` | `#E7E626` | Buttons, active states, focus ring |
| Secondary CTA | `cta-secondary` | `#507060` | Secondary button, Art Objects route |

### 3.2 Route Colors

Each of the five routes has a dedicated brand color used for route pills, dots on the map, icons, and section headers.

| Route | Hex | Role in SVG Style Guide |
|---|---|---|
| Typography | `#333DA7` | Medium blue |
| Architecture | `#9D372E` | Brick red |
| Art Objects (Artifacts) | `#257A48` | Forest green |
| Street Art (Murals) | `#8E2E8E` | Purple |
| Artist's Pick | `#0D7A82` | Dark teal |

### 3.3 Mid-Fi Token Map (Prototype / Development Reference)

Used in the Figma mid-fi prototype. These are neutral approximations — replace with HiFi values in final build.

| Token | Dark Mode | Light Mode Equivalent |
|---|---|---|
| `bg-base` | `#1E1E1E` | `#223740` (HiFi) |
| `bg-card` | `#2A2A2A` | `#2D4855` (HiFi) |
| `bg-elevated` | `#363636` | — |
| `border` | `#424242` | — |
| `text-primary` | `#C0C0C0` | `#EDF4F0` (HiFi) |
| `text-secondary` | `#525252` | `#64858D` (HiFi) |
| `cta-default` | `#4A6EAA` | `#E7E626` (HiFi) |
| `cta-hover` | `#5A80BE` | — |
| `cta-pressed` | `#365290` | — |
| `cta-focused` | Focus ring only, 3px outline, 2px offset | HiFi ring: `#E7E626` |
| `cta-disabled` | `#2A2A40` | — |
| `cta-text` | `#E8E8E8` | — |

**Route states (mid-fi dark mode):**

| Route | Default dot bg | Active dot | Active ring |
|---|---|---|---|
| Typography | `rgba(#7A8A90, 30%)` | `#90A8B4` | `#B8CCD4` |
| Architecture | `rgba(#8A6A60, 30%)` | `#A88070` | `#C8A098` |
| Artifacts | `rgba(#507060, 30%)` | `#688C78` | `#88AA98` |
| Street Art | `rgba(#5A4A60, 30%)` | `#706078` | `#9080A0` |
...
| Font | Role | Source |
|---|---|---|
| **ITC Avant Garde Gothic Pro** | Display, headings, UI labels, CTA | Licensed / Adobe Fonts |
| **Adelle** | Body copy, editorial long-form | Licensed / Adobe Fonts |
| **DM Sans** | Mid-fi prototype fallback | Google Fonts (free) |

**Fallback stack:** `"ITC Avant Garde Gothic Pro", "Futura", "Century Gothic", sans-serif`  
**Body fallback:** `"Adelle", "Georgia", serif`
...
|---|---|---|---|---|---|
| H1 | 28px | 700 Bold | 28px / 100% | `text-emphasis` (#E8E8E8) | Page titles |
| H2 | 22px | 700 Bold | 120% | `text-emphasis` | Section headers |
| H3 | 19px | 700 Bold | 120% | `text-emphasis` | Sub-section headers |
| H4 | 17px | 600 SemiBold | 130% | `#D0D0D0` | Must exceed 16px body |
| H5 | 16px | 500 Medium | 135% | `#A8A8A8` | Weight + color distinguish from body |
| P Body Primary | 16px | 400 Regular | 140% | `text-body` | Main reading text |
| P Body Primary Highlights | 16px | 600 SemiBold | 140% | `text-body` | Inline emphasis |
| P Body Secondary | 15px | 400 Italic | 130% | `text-secondary` | Captions, sub-labels |
| Caption 1 | 12px | 400 Regular | 120% | `text-secondary` | Metadata, timestamps |
| Caption 2 | 11px | 400 Regular | 120% | `text-secondary` | Floor: 11px (outdoor mobile conditions) |
...
| Nav Label | 10px | 400 Regular | 100% | `text-secondary` | Bottom nav labels (chrome, not copy) |
| Tags / Category | 9px | 500 Medium | 120% | `text-body` | Route tags, filter pills |
| Map Location | 11px | 400 Regular | 120% | `text-secondary` | Map overlays |
| Map Distance | 11px | 400 Regular | 120% | `text-secondary` | Distance metadata |
...
| Quiz Question | 17px | 600 SemiBold | 130% | `text-emphasis` | H4 style + emphasis color override |
| Quiz Hint | 15px | 400 Italic | 120% | `text-secondary` | = P Body Secondary |
...
- Quiz Question reuses H4 style with `text-emphasis` color override. No separate style needed.
...
| Quiz Question reuses H4 style | Avoids extra style; colour override (#text-emphasis) provides sufficient distinction |
