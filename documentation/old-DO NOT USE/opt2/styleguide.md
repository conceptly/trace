...
### Depth through teal
All backgrounds share the same blue-teal colour family. In dark mode they range from `#12202A` (deepest) to `#3D5E6A` (lightest surface). In light mode the range flips to `#F4F9FA` → `#BDD0D5`. The same tonal hierarchy applies in both modes — only the direction changes. This creates a cohesive environment where the background recedes and content surfaces naturally.
...
All contrast ratios measured against the primary screen background `#223740` using the WCAG 2.1 relative luminance formula.

### Backgrounds

| Token         | Hex       | Role                                   |
|---------------|-----------|----------------------------------------|
| Canvas        | `#12202A` | Outer page / app shell background      |
| Deep Surface  | `#1D3240` | Elevated panels, sidebar sections      |
| Screen BG     | `#223740` | Base screen background                 |
| Card / Surface| `#2D4855` | Cards, nav bars, bottom drawers        |
| Divider       | `#3D5E6A` | Hairlines, borders, outlines           |

### Text

| Token          | Hex       | Contrast on `#223740` | WCAG Grade |
|----------------|-----------|----------------------|------------|
| Text Primary   | `#EDF4F0` | 12.0 : 1             | AAA        |
| Text Secondary | `#BDD0CC` | 7.9 : 1              | AAA        |
| Text Muted     | `#8AADB5` | 5.3 : 1              | AA         |
...
| Logo TRACE    | `#E7E626` | TRACE wordmark fill                             |
| Logo TO       | `#EDF4F0` | "TO" suffix fill                                |
...
|--------------------|-----------|------------------------------|
| Top Nav BG         | `#2D4855` |                              |
| Bottom Nav BG      | `#223740` |                              |
| Active Icon        | `#EDF4F0` | Active tab icon + label      |
| Inactive Icon      | `#8AADB5` | 5.3 : 1 contrast ✓ AA        |
| Active Pill BG     | `#2D4855` | Pill behind active icon      |
...
All contrast ratios measured against the primary screen background `#E4EFF2`.

### Backgrounds

| Token         | Hex       | Role                                   |
|---------------|-----------|----------------------------------------|
| Canvas        | `#F4F9FA` | Outer page / app shell background      |
| Deep Surface  | `#EDF5F7` | Subtle elevated surface, alt sections  |
| Screen BG     | `#E4EFF2` | Base screen background                 |
| Card / Surface| `#FFFFFF` | Cards, nav bars, bottom drawers        |
| Divider       | `#BDD0D5` | Hairlines, borders, outlines           |

### Text

| Token          | Hex       | Contrast on `#E4EFF2` | WCAG Grade |
|----------------|-----------|----------------------|------------|
| Text Primary   | `#1D3240` | 11.4 : 1             | AAA        |
| Text Secondary | `#2D4855` | 7.5 : 1              | AAA        |
| Text Muted     | `#3A6070` | 5.1 : 1              | AA         |

### Brand & Interactive

| Token         | Hex       | Usage                                             |
|---------------|-----------|---------------------------------------------------|
| Logo TRACE    | `#1D3240` | Wordmark fill (yellow fails on light backgrounds) |
| Logo TO       | `#2D4855` | "TO" suffix fill                                  |
| Focus Ring    | `#1D3240` | 2px outline — 11.4 : 1 contrast ✓ AAA            |

> **Why the focus ring changes:** `#E7E626` on `#FFFFFF` achieves only ~1.9 : 1 contrast — a critical WCAG failure. The focus ring must switch to `#1D3240` in light mode to remain perceivable by keyboard users.
...
|--------------------|-----------|--------------------------------|
| Top Nav BG         | `#FFFFFF` |                                |
| Bottom Nav BG      | `#EDF5F7` |                                |
| Active Icon        | `#1D3240` | Active tab icon + label        |
| Inactive Icon      | `#3A6070` | 5.1 : 1 on `#EDF5F7` ✓ AA     |
| Active Pill BG     | `#DDE9EC` | Pill behind active icon        |
...
- Border: `1px solid` divider token
...
- Background: transparent
- Border: `1px solid` divider token
- Icon fill: text-muted token
- Size: 34×34px
- Border radius: 50% (pill)
- Focus ring: 2px solid focus-ring token

#### Icon Button — Active / Pressed
- Background (dark): `#BDD0CC`; Icon fill: `#2D4855`
- Background (light): surfaceAlt token; Icon fill: text-muted token
...
#### Top navigation bar
- Height: 64px
- Background: navTop token
- Border bottom: `1px solid` divider token
- Contains: Logo (left), icon button group (right)
- `<nav aria-label="Main navigation">`

#### Bottom tab bar
- Height: 64px
- Background: navBottom token
- Border top: `1px solid` divider token
- Three tabs: Map, Guide, Profile
- Active tab: icon in navActivePill, text + icon in navActiveIcon, `aria-current="page"`
- Inactive tab: icon + text in navInactiveIcon (5.1 : 1 minimum)
...
- Background (dark): surface token; (light): `#EEF5F7`
...
- Background: sheetBg token
- Border top: `1px solid` divider token
- Drag handle: 56×3px pill, drag-handle token colour, centred
- `role="region" aria-label="Spots near you"`

### Search bar
- `<label htmlFor="...">` with `sr-only` class wrapping visible input
- Placeholder text in searchText token colour
- Background: searchBg token; border: searchBorder token
- Height: 50px; border radius: 16px
- Caret colour: focusRing token
